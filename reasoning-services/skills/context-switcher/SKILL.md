---
description: Use when a decision affects multiple stakeholders or teams, designing APIs or interfaces, or when you need to surface blind spots from different roles. Runs parallel perspective analysis. Triggers on: 'different perspectives', 'who else is affected', 'stakeholder analysis', 'surface blind spots', 'how would X see this', 'multiple viewpoints', 'who would push back on this', 'see this from different angles', 'what are we missing', 'whose concerns aren't we hearing'. INTENSITY STEERING: when user says 'leave no stone unturned', 'comprehensive review', 'all angles', 'find the blind spots', 'I need buy-in from everyone', or 'thorough stakeholder analysis' → use 5–7 perspectives with specific focus areas (not just role names), use analyze_from_perspectives_stream, synthesize non-obvious cross-cutting concerns and second-order effects. When user says 'sanity check', 'quick gut check', or 'sense check' → use 3 perspectives (technical / user / business), use analyze_from_perspectives, synthesize top concern only. Default: 4 perspectives chosen for productive tension, use analyze_from_perspectives, synthesize agreements and genuine tensions.
---
