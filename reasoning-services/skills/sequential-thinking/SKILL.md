---
description: Use when a problem needs step-by-step linear reasoning with explicit confidence tracking, dependency mapping, and contradiction detection. Maintains an auditable reasoning chain. Triggers on: 'step by step', 'walk me through', 'think this through', 'trace this logic', 'linear reasoning', 'show your reasoning', 'build the argument', 'prove this', 'chain of reasoning', 'what follows from', 'if X then what'. INTENSITY STEERING: when user says 'deep dive', 'thorough', 'ultrathink', 'go deep', 'prove this', 'build the full argument', or 'I need to show my reasoning' → set total_thoughts to 10–15, run all 5 stages (Problem Definition → Information Gathering → Analysis → Synthesis → Conclusion), include 2+ explicit assumptions per step. When user says 'quick', 'fast', 'gut check', 'just trace this', 'high-level', or 'sketch this out' → set total_thoughts to 3–4, use Problem Definition → Analysis → Conclusion only, skip explicit assumption tracking. Default: total_thoughts 5–8, stages matched to problem shape, assumptions tracked as they arise.
---
